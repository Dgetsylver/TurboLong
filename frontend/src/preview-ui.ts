...
function section(title: string, ...nodes: (Node | string)[]): HTMLElement {
  return el("section", {}, [
    el("h2", { style: "font-size:13px;text-transform:uppercase;letter-spacing:.5px;color:var(--tl-text-3);margin:0 0 12px" }, [title]),
...
root.append(
  ...
  section("RiskBand",
    el("div", { style: "min-width:280px" }, [RiskBand({ zone: zoneFromHF(1.42) })]),
  ),
  section("LeverageSlider",
    el("div", { style: "min-width:420px" }, [LeverageSlider({ value: 3.0, onChange: () => {} })]),
  ),
  section("Tooltip",
    el("span", { style: "color:var(--tl-text-2)" }, ["Health Factor ", Tooltip({ text: "Σ(collateral × c_factor) ÷ Σ(debt). Below 1.0 = liquidation." })]),
  ),
  section("Skeleton",
    Skeleton({ width: 80 }),
...
const modal = Modal({
  open: false,
  title: "Confirm & open",
  children: el("p", { style: "color:var(--tl-text-2);font-size:13px" }, ["You may lose your entire deposit. Leverage amplifies liquidation risk."]),
