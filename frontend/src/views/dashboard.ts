...
export function renderDashboard(data: DashboardData, h: DashboardHandlers): HTMLElement {
  ...
  // Summary
  const totalEquity =
    data.poolAccounts.reduce((a, p) => a + p.equityUsd, 0) + data.vaults.reduce((a, v) => a + v.equityUsd, 0);
  ...
  const lowestHf = data.poolAccounts.length ? Math.min(...data.poolAccounts.map((p) => p.accountHealth)) : 0;
  ...
  root.append(
    el("div", { class: "tl-grid tl-grid--4 tl-dash__summary" }, [
      ...
      metricHero(
        ...
        lowestHf ? (Number.isFinite(lowestHf) ? lowestHf.toFixed(4) : "∞") : "—",
        hfColor(lowestHf || 99),
    ...
    root.append(
      el(
        ...
        data.poolAccounts.map((acc) => {
          ...
          const primary =
            acc.legs.find((l) => l.role === "Looped") ?? [...acc.legs].sort((a, b) => b.amountUsd - a.amountUsd)[0];
          return poolCard(
            acc,
...
export const SEED_DASHBOARD: DashboardData = {
  ...
  vaults: [{ name: "USDC · YieldBlox Vault", equityUsd: 3420, share: "0.12%", netApy: 18.6, strategyHealth: 1.62 }],
};
