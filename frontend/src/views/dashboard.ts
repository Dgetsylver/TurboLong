...
function metricHero(
  ...
  return el("div", { class: "tl-hero" }, [
    el("span", { class: "tl-hero__label", ...(title ? { title } : {}) }, [
      label,
    ]),
    el(
      "span",
      { class: "tl-hero__value", style: tone ? `color:${tone}` : "" },
      [value],
    ),
  ]);
}
...
function miniMetric(label: string, value: string, tone?: string): HTMLElement {
  return el("div", { class: "tl-mini" }, [
    ...
    el(
      "span",
      { class: "tl-mini__value", style: tone ? `color:${tone}` : "" },
      [value],
    ),
  ]);
}
...
function accountHealth(hf: number, title: string): HTMLElement {
  ...
  return el("div", { class: "tl-hf" }, [
    el("div", { class: "tl-hf__top" }, [
      ...
      el("span", { class: "tl-hf__val", style: `color:${color}` }, [
        (Number.isFinite(hf) ? hf.toFixed(4) : "∞") + " · " + hfLabel(hf),
      ]),
    ]),
...
function legRow(lg: Leg, last: boolean): HTMLElement {
  ...
  if (lg.loopX)
    left.append(
      el("span", { class: "tl-leg__sub" }, [lg.loopX.toFixed(1) + "× loop"]),
    );
...
function poolCard(
  ...
  if (cross) {
    ...
    xc.title =
      "More than one asset in this pool. They share one Account Health — every collateral backs every borrow, so liquidation is account-wide, not per leg.";
    actions.push(xc);
  }
  ...
  const card = el("section", { class: "tl-card" }, [
    ...
    el("div", { class: "tl-grid tl-grid--2" }, [
      ...
      miniMetric(
        "Net APY",
        pct(acc.netApy),
        acc.netApy >= 0 ? "var(--tl-success)" : "var(--tl-danger)",
      ),
    ]),
    el(
      "div",
      { class: "tl-legs" },
      acc.legs.map((lg, i) => legRow(lg, i === acc.legs.length - 1)),
    ),
  ...
  const manage = el("button", { class: "tl-btn tl-btn--secondary tl-flex1" }, [
    "Manage",
  ]);
  manage.addEventListener("click", onManage);
  const add = el("button", { class: "tl-btn tl-btn--ghost tl-flex1" }, [
    "Add leg",
  ]);
...
function vaultCard(v: VaultHolding, onManage: () => void): HTMLElement {
  ...
  managed.title =
    "A passive vault position. A permissionless keeper auto-rebalances the strategy, so you’re not liquidated the way a manual pool account is — the strategy health below is informational.";

  const manage = el("button", { class: "tl-btn tl-btn--secondary tl-full" }, [
    "Manage in Vault",
  ]);
  manage.addEventListener("click", onManage);
  ...
  return el("section", { class: "tl-card" }, [
    ...
    el("div", { class: "tl-strategy" }, [
      ...
      "Strategy Health ",
      el("span", { class: "tl-mono tl-strategy__hf" }, [
        Number.isFinite(v.strategyHealth) ? v.strategyHealth.toFixed(4) : "∞",
      ]),
      " · keeper-protected",
    ]),
    manage,
...
export interface DashboardHandlers {
  ...
  onNewPosition: () => void;
  /** "Export CSV" header button — download current positions and event history. */
  onExportCsv: () => void;
  /** "Export JSON" header button — download current positions and event history. */
  onExportJson: () => void;
...
export function renderDashboard(
  ...
  if (
    ...
    return el("section", { class: "tl-card tl-empty" }, [
      "Connect your wallet to see your positions.",
    ]);
  }

  // Header
  const exportCsv = el("button", { class: "tl-btn tl-btn--ghost tl-btn--sm" }, [
    "Export CSV",
  ]);
  exportCsv.addEventListener("click", h.onExportCsv);
  const exportJson = el(
    "button",
    { class: "tl-btn tl-btn--ghost tl-btn--sm" },
    ["Export JSON"],
  );
  exportJson.addEventListener("click", h.onExportJson);
  const newBtn = el("button", { class: "tl-btn tl-btn--primary" }, [
    "+ New Position",
  ]);
  newBtn.addEventListener("click", h.onNewPosition);
  root.append(
    el("div", { class: "tl-dash__head" }, [
      el("div", {}, [
        el("h1", { class: "tl-h1" }, ["Dashboard"]),
        el("p", { class: "tl-sub" }, [
          "Your accounts, grouped by pool. Health is shared across every leg in a pool.",
        ]),
      ]),
      el("div", { class: "tl-row tl-row--gap" }, [
        exportCsv,
        exportJson,
        newBtn,
      ]),
  ...
  // Summary
  const totalEquity =
    data.poolAccounts.reduce((a, p) => a + p.equityUsd, 0) +
    data.vaults.reduce((a, v) => a + v.equityUsd, 0);
  const poolEquity =
    data.poolAccounts.reduce((a, p) => a + p.equityUsd, 0) || 1;
  const wAvgApy =
    data.poolAccounts.reduce((a, p) => a + p.netApy * p.equityUsd, 0) /
    poolEquity;
  ...
  root.append(
    el("div", { class: "tl-grid tl-grid--4 tl-dash__summary" }, [
      metricHero(
        "Total Equity",
        money(totalEquity),
        undefined,
        "Your own capital across all pools and vaults — collateral value minus debt, summed.",
      ),
      metricHero(
        "Avg Net APY",
        pct(wAvgApy),
        wAvgApy >= 0 ? "var(--tl-success)" : "var(--tl-danger)",
      ),
      metricHero(
        "Positions",
        String(data.poolAccounts.length + data.vaults.length),
        undefined,
        "Your active pool accounts plus passive vault positions.",
      ),
      metricHero(
        "Lowest Account Health",
        lowestHf
          ? Number.isFinite(lowestHf)
            ? lowestHf.toFixed(4)
            : "∞"
          : "—",
        hfColor(lowestHf || 99),
        "The riskiest pool account. Below 1.0 the whole pool account is liquidated.",
      ),
    ]),
  );
  ...
  if (data.poolAccounts.length) {
    root.append(
      el("h2", { class: "tl-group" }, [
        "Pool Accounts ",
        el("span", { class: "tl-group__sub" }, ["active · self-managed"]),
      ]),
    );
    root.append(
      el(
        "div",
        { class: "tl-grid tl-grid--2" },
  ...
  if (data.vaults.length) {
    root.append(
      el("h2", { class: "tl-group tl-group--mt" }, [
        "Vaults ",
        el("span", { class: "tl-group__sub" }, ["passive · auto-rebalanced"]),
      ]),
    );
    root.append(
      el(
