...
const MAINNET_CONFIG: NetworkConfig = {
  ...
  classicAssets: {
    ...
    CCW67TSZV3SSS2HXMBQ5JFGCKJNXKZM7UQUWUZPUTHXSTZLEO7SJMI75: new Asset(
      "USDC",
      "GA5ZSEJYB37JRC5AVCIA5MOP4RHTM335X2KGX3IHOJAPP5RE34K4KZVN",
    ),
    CDTKPWPLOURQA2SGTKTUQOWRCBZEORB4BWBOMJ3D3ZTQQSGE5F6JBQLV: new Asset(
      "EURC",
      "GDHU6WRG4IEQXM5NZ4BMPKOXHW76MZM4Y2IEMFDVXBSDP6SJY4ITNPP2",
    ),
    CAUIKL3IYGMERDRUN6YSCLWVAKIFG5Q4YJHUKM4S4NJZQIA3BAS6OJPK: new Asset(
      "AQUA",
      "GBNZILSTVQZ4R7IKQDGHYGY2QXL5QOFJYQMXPKWRRM5PAV7Y4M67AQUA",
    ),
    CB226ZOEYXTBPD3QEGABTJYSKZVBP2PASEISLG3SBMTN5CE4QZUVZ3CE: new Asset(
      "USDGLO",
      "GBBS25EGYQPGEZCGCFBKG4OAGFXU6DSOQBGTHELLJT3HZXZJ34HWS6XV",
    ),
    CBLV4ATSIWU67CFSQU2NVRKINQIKUZ2ODSZBUJTJ43VJVRSBTZYOPNUR: new Asset(
      "USTRY",
      "GCRYUGD5NVARGXT56XEZI5CIFCQETYHAPQQTHO2O3IQZTHDH4LATMYWC",
    ),
    CAL6ER2TI6CTRAY6BFXWNWA7WTYXUXTQCHUBCIBU5O6KM3HJFG6Z6VXV: new Asset(
      "CETES",
      "GCRYUGD5NVARGXT56XEZI5CIFCQETYHAPQQTHO2O3IQZTHDH4LATMYWC",
    ),
    CCCRWH6Q3FNP3I2I57BDLM5AFAT7O6OF6GKQOC6SSJNDAVRZ57SPHGU2: new Asset(
      "PYUSD",
      "GDQE7IXJ4HUHV6RQHIUPRJSEZE4DRS5WY577O2FY6YQ5LVWZ7JZTU2V5",
    ),
    CD6M4R2322BYCY2LNWM74PEBQAQ63SA3DUJLI3L4225U4ZVCLMSCBCIS: new Asset(
      "TESOURO",
      "GCRYUGD5NVARGXT56XEZI5CIFCQETYHAPQQTHO2O3IQZTHDH4LATMYWC",
    ),
  },
...
export function getKnownPools(): PoolDef[] {
  return _cfg.pools;
}
export function getBlndId(): string {
  return _cfg.blndId;
}
...
function assetScVal(contractId: string): xdr.ScVal {
  return xdr.ScVal.scvVec([xdr.ScVal.scvSymbol("Stellar"), new Address(contractId).toScVal()]);
}
...
export async function fetchBlndPrice(pool: PoolDef, userAddress: string): Promise<number> {
  ...
  // CoinGecko free API
  try {
    const res = await fetch("https://api.coingecko.com/api/v3/simple/price?ids=blend&vs_currencies=usd", {
  ...
  for (const asset of assets) {
    ...
    try {
      const calls: Promise<any>[] = [
        ...
        oracle ? simulate(oracle.call("lastprice", assetScVal(asset.id))) : Promise.resolve(null),
    ...
    const supplyEps =
      supplyEmissions?.eps != null && Number(supplyEmissions.expiration ?? 0) > nowSec
        ? BigInt(supplyEmissions.eps)
        : 0n;
    const borrowEps =
      borrowEmissions?.eps != null && Number(borrowEmissions.expiration ?? 0) > nowSec
        ? BigInt(borrowEmissions.eps)
        : 0n;
    ...
    console.log(
      `[blend:${pool.name}] ${asset.symbol} util=${util.toFixed(4)} c=${cFactor.toFixed(4)} l=${lFactor.toFixed(4)} borrowApr=${interestBorrowApr.toFixed(4)}% supplyApr=${interestSupplyApr.toFixed(4)}% blndSupplyApr=${blndSupplyApr.toFixed(4)}% supplyEps=${supplyEps}`,
    );
...
export async function fetchUserPositions(
  ...
  const poolContract = new Contract(pool.id);
  const raw = await simulate(poolContract.call("get_positions", new Address(userAddress).toScVal()));
...
export function aggregatePoolAccount(positions: AssetPosition[], reserves: ReserveStats[]): PoolAccountSummary {
  ...
  for (const pos of positions) {
    ...
    const role: PoolAccountRole = pos.collateral > 0 && pos.debt > 0 ? "loop" : pos.debt > 0 ? "borrow" : "collateral";
...
export async function fetchAssetBalance(userAddress: string, assetId: string): Promise<number> {
  ...
  const raw = await simulate(token.call("balance", new Address(userAddress).toScVal()));
...
export const MIN_POST_LOOP_LIQUIDITY = 0.1;
 ...
 */
export function checkLoopSafety(rs: ReserveStats, initialAmount: number, leverage: number): LoopSafetyResult {
  ...
  if (rateSpreadPct > MAX_RATE_SPREAD_PCT) {
    ...
    warnings.push(`Borrow-supply spread is ${rateSpreadPct.toFixed(1)}%/yr — HF will erode quickly.`);
  }
...
export async function buildApproveXdr(
  ...
  const tx = new TransactionBuilder(acc, {
    ...
    .addOperation(
      token.call("approve", addrScVal, poolScVal, i128ToScVal(amountStroops), nativeToScVal(expiry, { type: "u32" })),
    )
    .setTimeout(60)
    .build();
...
export async function buildOpenPositionXdr(
  ...
  if (!SorobanRpc.Api.isSimulationSuccess(sim))
    throw new Error(
      `Open position simulation failed: ${(sim as SorobanRpc.Api.SimulateTransactionErrorResponse).error}`,
    );
  return SorobanRpc.assembleTransaction(tx, sim).build().toXDR();
}
 ...
 */
export async function buildCloseSubmitXdr(pool: PoolDef, userAddress: string, pos: AssetPosition): Promise<string> {
 ...
 */
export async function buildRepayXdr(pool: PoolDef, userAddress: string, pos: AssetPosition): Promise<string> {
 ...
 */
export async function buildWithdrawXdr(pool: PoolDef, userAddress: string, assetId: string): Promise<string> {
  const MAX_AMOUNT = 9_223_372_036_854_775_807n;
  const requests = buildRequestsVec([buildRequest(assetId, MAX_AMOUNT, WITHDRAW_COLLATERAL)]);
...
export async function fetchPoolPendingBlnd(
  ...
  const poolContract = new Contract(pool.id);
  const addrScVal = new Address(userAddress).toScVal();
  const tokenIdsScVal = xdr.ScVal.scvVec(tokenIds.map((id) => nativeToScVal(id, { type: "u32" })));
  const result = await simulate(poolContract.call("claim", addrScVal, tokenIdsScVal, addrScVal));
  if (result === null) return 0;
  const stroops = typeof result === "bigint" ? result : BigInt(result as any);
  return Number(stroops > 0n ? stroops : 0n) / SCALAR_F;
}

export async function buildClaimXdr(pool: PoolDef, userAddress: string, tokenIds: number[]): Promise<string> {
  const poolContract = new Contract(pool.id);
  const addrScVal = new Address(userAddress).toScVal();
  const tokenIds_scv = xdr.ScVal.scvVec(tokenIds.map((id) => nativeToScVal(id, { type: "u32" })));
...
export async function buildIncreaseLeverageXdr(
  ...
  const firstBorrow =
    ...
    (pos.collateral * 1e7 > 0
      ? (BigInt(Math.round(pos.collateral * 1e7)) * cFactorBn) / SCALAR - BigInt(Math.round(pos.debt * 1e7))
      : 0n)
      ? additionalBorrowStroops
      : additionalBorrowStroops;
  ...
  if (!SorobanRpc.Api.isSimulationSuccess(sim))
    throw new Error(
      `Increase leverage simulation failed: ${(sim as SorobanRpc.Api.SimulateTransactionErrorResponse).error}`,
    );
  return SorobanRpc.assembleTransaction(tx, sim).build().toXDR();
}
...
export async function buildDecreaseLeverageXdr(
  ...
  if (!SorobanRpc.Api.isSimulationSuccess(sim))
    throw new Error(
      `Decrease leverage simulation failed: ${(sim as SorobanRpc.Api.SimulateTransactionErrorResponse).error}`,
    );
  return SorobanRpc.assembleTransaction(tx, sim).build().toXDR();
}
...
export async function buildRemoveFundsXdr(
  ...
  if (!SorobanRpc.Api.isSimulationSuccess(sim))
    throw new Error(
      `Remove funds simulation failed: ${(sim as SorobanRpc.Api.SimulateTransactionErrorResponse).error}`,
    );
  return SorobanRpc.assembleTransaction(tx, sim).build().toXDR();
}
...
export async function buildResupplyXdr(
  ...
  const requests = buildRequestsVec([buildRequest(assetId, amountStroops, SUPPLY_COLLATERAL)]);
...
export async function buildSwapBlndXdr(
  ...
  const paths = await horizon.strictSendPaths(_cfg.blndClassic, sendAmount, [destAsset]).call();
...
export async function estimateBlndSwap(
  ...
  try {
    const paths = await horizon.strictSendPaths(_cfg.blndClassic, blndAmount.toFixed(7), [destAsset]).call();
    ...
    return {
      ...
      path: via
        ? `BLND → ${via} → ${destAsset.isNative() ? "XLM" : destAsset.getCode()}`
        : `BLND → ${destAsset.isNative() ? "XLM" : destAsset.getCode()}`,
    };
  } catch {
    return null;
  }
...
export async function submitSignedXdr(signedXdr: string): Promise<string> {
  ...
  if (result.status === "ERROR") throw new Error(`Send failed: ${result.errorResult?.toXDR("base64")}`);

  for (let i = 0; i < 30; i++) {
    await new Promise((r) => setTimeout(r, 3000));
    const poll = await withRetry(() => server.getTransaction(result.hash));
    if (poll.status === "SUCCESS") return result.hash;
    if (poll.status === "FAILED") throw new Error(`On-chain failure: ${poll.resultXdr?.toXDR("base64")}`);
  }
...
export async function getMissingTrustlines(
  ...
  const poolContract = new Contract(pool.id);
  const reserveRaw = await simulate(poolContract.call("get_reserve", new Address(assetId).toScVal()));
  if (!reserveRaw) {
  ...
  const existing = new Set<string>(
    acc.balances.filter((b: any) => b.asset_type !== "native").map((b: any) => `${b.asset_code}:${b.asset_issuer}`),
  );
 ...
 */
export async function buildTrustlineXdr(missingAssets: Asset[], userAddress: string): Promise<string | null> {
