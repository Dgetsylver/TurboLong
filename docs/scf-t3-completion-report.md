...
|---|-------------|--------|----------|
| T3.1 | Aquarius rate comparison + Best Rate UI | ✅ Built / ✅ verified live | PR #273 — `aquarius.ts` (find-path client), `history.ts` (server-backed trends), swap-view Aquarius rate + Best Rate badge. Acceptance: **8 mainnet pairs priced live** (≥5 required), all stability/round-trip/depth checks PASS — `docs/evidence/aquarius-rate-report.md` (regenerate: `npx tsx scripts/aquarius_rate_report.ts`). 24h/7d trend arrows from the T2 `rate_snapshots` table. Fallback documented: `docs/aquarius-rate-fallback.md`. Unit tests: `frontend/test/aquarius.test.ts` (17), `frontend/test/compare_metrics.test.ts` (18) |
...
| T3.3 | Compare Pools view (cross-pool APY + Aquarius + history) | ✅ Built | PR #274 — no-wallet Compare view, ranked leveraged net APY, Aquarius rates, 24H/7D/30D/1Y history sparklines, 24h+7d delta arrows, Best Rate badge (issue #11) |
...
- Aquarius rate fallback / degraded mode: `docs/aquarius-rate-fallback.md`
- Aquarius rate acceptance evidence: `docs/evidence/aquarius-rate-report.md`
