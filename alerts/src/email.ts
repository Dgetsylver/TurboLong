...
export async function sendRateSpikeAlert(
  env: Env,
  to: string,
  opts: {
    poolName: string;
  ...
  const { poolName, assetSymbol, borrowAprBefore, borrowAprAfter, deltapp, thresholdPp, unsubscribeUrl, appUrl } = opts;
...
<head><meta charset="utf-8"></head>
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: 480px; margin: 0 auto; padding: 32px 16px; color: #1a1a2e;">
  <h2 style="margin: 0 0 8px; color: #FF4D6A;">&#9889; Borrow Rate Spike</h2>
  <p style="font-size: 14px; color: #555; margin: 0 0 20px;">${assetSymbol} on ${poolName} — borrow rate jumped <strong>+${deltapp.toFixed(2)}pp</strong> in the last 15 minutes (threshold: ${thresholdPp.toFixed(1)}pp).</p>

  <div style="background: #f8f8fc; border-radius: 8px; padding: 16px; margin-bottom: 20px;">
    <table style="width: 100%; font-size: 14px; border-collapse: collapse;">
      <tr><td style="padding: 4px 0; color: #555;">Borrow APR before</td><td style="padding: 4px 0; text-align: right; font-weight: 600;">${borrowAprBefore.toFixed(2)}%</td></tr>
      <tr><td style="padding: 4px 0; color: #555;">Borrow APR now</td><td style="padding: 4px 0; text-align: right; font-weight: 600; color: #FF4D6A;">${borrowAprAfter.toFixed(2)}%</td></tr>
      <tr style="border-top: 1px solid #e0e0e8;"><td style="padding: 8px 0 4px; font-weight: 600;">Change</td><td style="padding: 8px 0 4px; text-align: right; font-weight: 700; color: #FF4D6A;">+${deltapp.toFixed(2)}pp</td></tr>
    </table>
  </div>

  <p style="line-height: 1.6; color: #555;">A sudden rate spike may indicate high pool utilization or unusual activity. Review your position.</p>

  <a href="${appUrl}" style="display: inline-block; margin: 16px 0; padding: 12px 28px; background: #2DE8A3; color: #0B0E14; text-decoration: none; border-radius: 8px; font-weight: 600;">Open Turbolong</a>

  <p style="font-size: 12px; color: #aaa; margin-top: 32px;">
    <a href="${unsubscribeUrl}" style="color: #aaa;">Unsubscribe</a> from rate-spike alerts.
  </p>
</body>
</html>`.trim();
  ...
  return sendEmail(
    env,
    to,
    `\u26A1 Rate spike: ${assetSymbol} borrow +${deltapp.toFixed(2)}pp on ${poolName}`,
    html,
  );
}
